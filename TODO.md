...
1. Screen-space god rays — (occlusion mask → radial blur → Kawase → composite)
   ...
   **Step 2 — Radial blur pass (light shaft)** ✅ DONE
   ...
   **Step 3 — Kawase blur pass (suavizado)** ✅ DONE

   - [x] Shader `god_rays_kawase.fs` — 4 taps con offsets para suavizar bandas radiales
   - [x] **4-5 passes** de Kawase en ping-pong con offsets `0, 1, 2, 2, 3` — dos passes son insuficientes y dejan bandas visibles a ¼ resolución
   - [x] RenderTargets ping-pong (`rgba8unorm`, tamaño ¼ — pueden reutilizarse de bloom si mismo formato/size)

   **Step 4 — Composite pass** ✅ DONE

   - [x] Shader `god_rays_composite.fs` — additive blend del resultado Kawase sobre el frame **HDR** (antes de tone mapping)
   - [x] Color del sol: **samplear la cubemap de entorno (`environmentTexture`) en la dirección del sol** — un uniform fijo se desincroniza con ciclos día/noche y cambios de color. Si no hay cubemap, fallback a uniform tint
   - [x] Opción `blendMode`: additive puro (por defecto) o multiplicativo con tint

   **Step 5 — Integración en el motor** ✅ DONE
