...
fn CS_advanced_blur_vertical(@builtin(global_invocation_id) global_id: vec3<u32>) {
    ...
    // 7-tap vertical blur: center + 3 taps on each side
    let c0 = textureSampleLevel(inputTexture, textureSampler, uv, 0.0).rgb;
    let c_p1 = textureSampleLevel(inputTexture, textureSampler, uv - vec2<f32>(0.0, blur_dir_y * d_first), 0.0).rgb;
    let c_n1 = textureSampleLevel(inputTexture, textureSampler, uv + vec2<f32>(0.0, blur_dir_y * d_first), 0.0).rgb;
    let c_p2 = textureSampleLevel(inputTexture, textureSampler, uv - vec2<f32>(0.0, blur_dir_y * d_second), 0.0).rgb;
    let c_n2 = textureSampleLevel(inputTexture, textureSampler, uv + vec2<f32>(0.0, blur_dir_y * d_second), 0.0).rgb;
    let c_p3 = textureSampleLevel(inputTexture, textureSampler, uv - vec2<f32>(0.0, blur_dir_y * d_third), 0.0).rgb;
    let c_n3 = textureSampleLevel(inputTexture, textureSampler, uv + vec2<f32>(0.0, blur_dir_y * d_third), 0.0).rgb;
