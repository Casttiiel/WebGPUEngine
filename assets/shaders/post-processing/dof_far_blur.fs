// Far Blur Pass - Aplica blur solo a píxeles del background (CoC positivo)
...
fn fs(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    ...
    let farCoC = cocData.r; // Canal R contiene far CoC
    ...
    let blurRadius = farCoC * texelSize.y;
    ...
    for (var i = 0; i < 12; i++) {
        ...
        let sampleFarCoC = sampleCoCData.r;
        
        // Weight: píxeles con más blur contribuyen más (evita bleeding del foreground)
        let weight = select(0.2, 1.0, max(sampleFarCoC, farCoC) > 0.5);
    ...
    // Si no hay blur en background, retornar color original; sino, retornar blur
    return select(originalColor, blurredColor, farCoC >= 0.5);
}
