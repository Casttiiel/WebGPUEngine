...
fn fs(@location(0) uv: vec2<f32>) -> @location(0) f32 {
    let radius = ssaoParams.radius;//1
    let step = 0.004;
    let tangentBias = ssaoParams.bias;//0.3
    let aoStrength = ssaoParams.aoStrength;
    let samplingDirections = ssaoParams.sampleCount;//6
    let stepCount = 4u;
    ...
    for (var i = 0u; i < samplingDirections; i = i + 1u) {
        ...
        for (var j = 0u; j < stepCount; j = j + 1u) {
            ...
            let stepPosition = uv + stepForward;
            let viewSpaceSteppedPosition = getViewPosition(stepPosition);
        ...
        let visibility = max(0.0, sin(horizonAngle) - sin(tangentAngle));
        let norm = clamp(length(lastDifference) / radius, 0.0, 1.0);
        let attenuation = 1.0 - norm * norm;
        let occlusion = attenuation * visibility;

        sum += occlusion;
    }

    let ao = 1.0 - (sum / f32(samplingDirections)) * aoStrength;
