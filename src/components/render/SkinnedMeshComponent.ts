...
export class SkinnedMeshComponent extends Component {
  ...
  // ── Static extra-animation cache ──────────────────────────────────────────
  // Keyed by filename (e.g. "Kick.gltf"). Populated on first load, reused after.
  ...
  public async load(data: SkinnedMeshComponentData): Promise<void> {
    ...
    // ── Obtain shared parsed asset (cached after first load) ─────────────────
    const asset = await SkinnedMeshAsset.get(gltfName);

    // Shared read-only references
    this.skeleton = asset.skeleton;
    this.skinnedMeshes = asset.skinnedMeshes; // shared GPU vertex buffers
    this.material = asset.material; // already cached by Material.get()
    this.evalOrder = asset.evalOrder;
    this.jointCount = this.skeleton.joints.length;
    this.rootJointIndex = asset.rootJointIndex;

    // Copy rootPreRotation (quat, per-instance mutable usage)
    quat.copy(this.rootPreRotation, asset.rootPreRotation as unknown as quat);

    // Base clips: shallow copy so extra animations can be appended per-instance
    this.clips = [...asset.clips];

    // ── Preallocate per-instance TRS working buffers ──────────────────────────
    ...
    // ── Extra animations (also cached after first parse) ──────────────────────
  ...
  private async loadExtraAnimations(paths: string[]): Promise<void> {
    // Build name → joint-index map from the existing skeleton (lowercase for case-insensitive match)
    ...
    for (const path of paths) {
      // Return cached clips immediately if already parsed
