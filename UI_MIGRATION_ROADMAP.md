...
### **FASE 2: Widgets Fundamentales** 🧩 [3-4 días]

#### 2.1 Widget Base (Refactorización)

**Objetivo**: Completar la implementación del Widget base.
...
- [ ] Integrar Transform2D en Widget
- [ ] Implementar `start()`, `stop()`, `update(dt)`, `render()`
- [ ] Sistema de jerarquía completo (setParent, removeFromParent)
- [ ] Gestión de efectos (add, remove, update)
- [ ] Método `childAppears()` para animaciones de fade-in
- [ ] Métodos de visibilidad y activación

#### 2.2 ImageWidget

**Objetivo**: Widget para renderizar sprites/texturas.
...
- [ ] Hereda de Widget base
- [ ] Gestión de `ImageParams`
- [ ] Soporte para UV coordinates (sprite sheets)
- [ ] Soporte para tintado de color
- [ ] Modo aditivo para efectos de brillo
- [ ] Método `render()` con integración a UIRenderUtils

#### 2.3 TextWidget

**Objetivo**: Widget para renderizar texto con bitmap fonts.
...
- [ ] Hereda de Widget base
- [ ] Gestión de `TextParams`
- [ ] Renderizado caracter por caracter desde bitmap font
- [ ] Layout ASCII estándar (8x8 grid)
- [ ] Soporte para color y tamaño

#### 2.4 ButtonWidget

**Objetivo**: Widget interactivo con estados.
...
- [ ] Hereda de Widget base
- [ ] Sistema de estados (normal, hover, pressed, disabled)
- [ ] Cada estado tiene su propia `ImageParams` y `TextParams`
- [ ] Método `setCurrentState(stateName: string)`
- [ ] Integración con sistema de input (ModuleInput)
- [ ] Callbacks para eventos (onClick, onHover, onPress)

#### 2.5 ProgressWidget y BarWidget

**Objetivo**: Widgets para barras de progreso/vida/stamina.

**Archivos a crear**:

- `src/components/ui/widgets/ProgressWidget.ts`
- `src/components/ui/widgets/BarWidget.ts`

**Tareas**:

- [ ] Hereda de Widget base
- [ ] Propiedad `ratio` (0.0 a 1.0)
- [ ] Renderizado con clipping/scaling según ratio
- [ ] Soporte para fondos y rellenos

#### 2.6 SpriteWidget

**Objetivo**: Widget para animaciones de sprite sheets con control frame-by-frame.
...
- [ ] Hereda de ImageWidget
- [ ] **Sistema multi-sprite**: Soporte para múltiples sprite sheets en un widget
- [ ] Propiedades por sprite:
  - `textures: Texture[]` - Array de sprite sheets
  - `originalImageSize: vec2[]` - Tamaño de cada sprite sheet
  - `frameSize: vec2[]` - Tamaño de cada frame individual
  - `numFrames: number[]` - Total de frames por sprite
  - `framesPerSecond: number[]` - FPS de animación configurable
  - `playingSprite: number` - Índice del sprite activo
- [ ] **Animación automática en `update(dt)`**:
  - Acumulador de tiempo basado en FPS
  - Avance de frame horizontal/vertical
  - Loop automático al final
  - Wrap-around de frames
- [ ] **Cálculo automático de UVs**:
  - Layout de grid (frames por fila × frames por columna)
  - Cálculo de minUV/maxUV por frame actual
  - Soporte para sprite sheets no uniformes
- [ ] Método `initializeSprite()` para configurar frame inicial
- [ ] Método `setPlayingSprite(index: number)` para cambiar sprite activo
...
export class SpriteWidget extends ImageWidget {
  ...
  public update(dt: number): void {
    ...
    const currentSprite = this.spriteParams.playingSprite;
    const frameDuration = 1.0 / this.spriteParams.framesPerSecond[currentSprite];
  ...
  private advanceFrame(): void {
    const sprite = this.spriteParams.playingSprite;
    const originalSize = this.spriteParams.originalImageSize[sprite];
    const frameSize = this.spriteParams.frameSize[sprite];

    this.actualHorizontalFrame++;
    this.actualFrame++;

    // Reset si llegamos al final
    if (this.actualFrame > this.spriteParams.numFrames[sprite]) {
      this.actualFrame = 1;
    ...
    // Siguiente fila
    const framesPerRow = Math.floor(originalSize[0] / frameSize[0]);
  ...
  private updateUVs(): void {
    ...
    const minUV = vec2.fromValues(
      (frameSize[0] / originalSize[0]) * this.actualHorizontalFrame,
      (frameSize[1] / originalSize[1]) * this.actualVerticalFrame,
    );

    const maxUV = vec2.fromValues(
      (frameSize[0] / originalSize[0]) * (this.actualHorizontalFrame + 1),
      (frameSize[1] / originalSize[1]) * (this.actualVerticalFrame + 1),
    );

    this.imageParams.minUV = minUV;
    this.imageParams.maxUV = maxUV;
  }
}
```
...
#### 3.4 Efectos Adicionales (Opcional)

**Objetivo**: Más efectos visuales si se necesitan.

**Ideas**:

- `FXRotate` - Rotación animada
- `FXFade` - Fade in/out de alpha
- `FXPosition` - Animación de posición (tweening)
- `FXPulse` - Efecto de pulsación/respiración
