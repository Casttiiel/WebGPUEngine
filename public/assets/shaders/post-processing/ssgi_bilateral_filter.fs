...
// SSGI texture sin filtrar (input) - 4 canales RGB+A
@group(2) @binding(0) var ssgiTexture: texture_2d<f32>;
@group(2) @binding(1) var samplerSSGI: sampler;

// Bilateral filter para SSGI (4 canales)
fn bilateralFilter(centerUV: vec2<f32>) -> vec4<f32> {
    var filteredSSGI = vec3<f32>(0.0);
    var totalWeight = 0.0;
    ...
    for (var x = -i32(BILATERAL_RADIUS); x <= i32(BILATERAL_RADIUS); x++) {
        for (var y = -i32(BILATERAL_RADIUS); y <= i32(BILATERAL_RADIUS); y++) {
            ...
            // Acumular los 3 canales RGB del SSGI
            filteredSSGI += sampleSSGI.rgb * weight;
            totalWeight += weight;
        }
    }
    
    // Normalizar por el peso total o usar valor original
    let finalSSGI = select(centerSSGI.rgb, filteredSSGI / totalWeight, totalWeight > 0.0);
    return vec4<f32>(finalSSGI, 1.0);
}

@fragment
fn fs(@location(0) uv: vec2<f32>) -> @location(0) vec4<f32> {
    // Apply bilateral filter to the SSGI texture
    let filteredSSGI = bilateralFilter(uv);
    // Output filtered SSGI RGB values with alpha = 1.0
    return filteredSSGI;
}
